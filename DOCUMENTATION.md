...
- [2026-07-17 08:30:51+0200]: Repository reorganisation — web app moved from root into `web-app/`
  ...
  - *Tech Notes*:
    ...
    - **Untracked (kept on disk)**: `dist/` (12 files) and `.DS_Store`, via `git rm --cached`. `dist/` was a committed duplicate of the deploy artifact — the live site is served from the `gh-pages` branch, which already holds that exact file set, and `predeploy` rebuilds it anyway. It had also been swept into unrelated Apple commits (e.g. `8b95230`).
    ...
    - **Discovered, not fixed**: `.github/workflows/deploy.yml` was deleted in commit `1e1b7cc` (2026-06-22) whose message is "icloud fix" — an apparently accidental removal of CI in an unrelated commit. Deploys have been manual ever since. Logged in `TO_SIMO_DO.md` as a decision rather than fixed here, to keep this change structural.
