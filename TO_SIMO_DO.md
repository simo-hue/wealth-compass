...
**2. Translate the new UI strings (they're already in the catalog).** The six new user-facing strings ("Import Data", "Import Data...", "Detected format: %@.", "The statement file could not be read. %@", the unrecognized-format error, and the reworded no-records error) were added to `Localizable.xcstrings` as `extractionState: manual` source entries. **Run your existing translation script/pipeline** to fill in the ~34 languages — until then they display in English (which works fine). The detected-format *names* ("Revolut consolidated statement", "Trade Republic transaction export") are intentionally shown verbatim (product/format names), and the imported category names (Investments/Shopping/…) are data, not UI strings. *(Number-format handling is DONE — the parser now accepts both English `1,234.56` and European `1.234,56`, so a German TR export won't be mis-parsed.)*
