...
> 🟡 **Step 1 done 2026-06-23; step 2 deferred.** **Location correction:** the non-deleted conflict path is *not* `resolveServerConflict` — that's only invoked for **deleted** server conflicts. The live code is the `nonDeletedConflicts` batch block in `handleSentRecordZoneChanges`, which adopted `systemFields` and then requeued **every** pending record.
> - **Step 1 (done):** each conflicting server record is now compared to the current local value via the shared `bootstrapDecision`; when it resolves `.identical` (same payload hash) we adopt the server's system fields and **clear pending** instead of re-uploading identical data. That removes the redundant re-uploads behind the concurrent-enable "hundreds of collisions" churn. The `.identical` decision is covered by the `bootstrapDecision` unit tests; the surrounding wiring isn't unit-testable (`CKSyncEngine` can't be instantiated here) and rests on build + the 2-device verify pass.
> - **Step 2 (deferred):** applying a *newer* server payload on genuine divergence (vs. today's requeue-and-re-upload-local) adds an async remote apply into the delicate send-failure path for a narrow concurrent-write race. Deliberately deferred so the change landing right before manual 2-device testing can only ever *drop a redundant upload* — worst case it falls back to the existing requeue, with no data-loss surface. Pick up as a focused follow-up.
...
*Last updated: 2026-06-23 — #15 step 1 (drop redundant upload on identical-payload conflict; step 2 deferred) + #8 (fetch-first bootstrap: confirmed already shipped in f91b0dc; added bootstrapDecision unit tests) + #6 (skip payload-less records, no fatal stop; scope A) + #7 (foreground sync API split + debounce) landed. 2026-06-22: M4 (off-main-actor save pipeline) landed; backlog status reconciled. Originally from the 2026-06-20 sync investigation session (macOS structured logs + iOS console).*
